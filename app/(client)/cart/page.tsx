...
type PaymentMethod = "bkash" | "cod" | "stripe";
...
const CartPage = () => {
  ...
  // Handle checkout for different payment methods
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (paymentMethod === "stripe") {
        ...
        const bkashPaymentUrl = await createBkashPayment(groupedItems, metadata);
        if (bkashPaymentUrl) {
          window.location.href = bkashPaymentUrl;
        }
      } else if (paymentMethod === "cod") {
        // Flatten payload to match backend expectation
        const payload = {
          orderNumber: metadata.orderNumber,
          customerName: metadata.customerName,
          customerEmail: metadata.customerEmail,
          clerkUserId: metadata.clerkUserId,
          address: metadata.address, // address at top level
          products: groupedItems.map(({ product, quantity }) => ({
