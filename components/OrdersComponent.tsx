...
const OrdersComponent = ({ orders }: { orders: MY_ORDERS_QUERYResult }) => {
  ...
  // Helper: calculate delivery date string (3 days after orderDate)
  const getDeliveryDate = (orderDate?: string) => {
    if (!orderDate) return "N/A";
    try {
      const date = addDays(new Date(orderDate), 3);
            ...
            <Tooltip key={order?.orderNumber}>
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  <TableCell className="hidden md:table-cell">{order?.orderDate ? format(new Date(order.orderDate), "dd/MM/yyyy") : "N/A"}</TableCell>
                  <TableCell className="hidden md:table-cell">{getDeliveryDate(order.orderDate)}</TableCell>
